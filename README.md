...
이 프로젝트는 **ChromaDB의 메타데이터 필터링 기본 기능을 활용**하면서도 **성능 최적화를 위한 이중 컬렉션 구조**를 채택했습니다.

#### 설계 배경 및 기술적 고민

**초기 설계 고민:**
- ChromaDB는 메타데이터 기반 필터링을 기본 제공하지만, 지원자 목록 조회를 위해 매번 전체 컬렉션의 메타데이터를 스캔하는 것은 비효율적입니다.
- 지원자 수가 증가할수록 전체 메타데이터 로드 시간이 선형적으로 증가하여, UI 렌더링과 질문 처리 시 지연이 발생합니다.

**최종 설계 결정:**
- **본문 컬렉션 (`chroma_db`)**: 모든 지원자의 청크를 단일 컬렉션에 저장하고, 각 청크의 메타데이터에 `candidate` 필드를 포함합니다. 검색 시 `filter={"candidate": "지원자명"}`을 사용하여 ChromaDB의 기본 메타데이터 필터링 기능을 활용합니다.
- **인덱스 컬렉션 (`candidates_index`)**: 지원자 목록만을 저장하는 경량 인덱스 컬렉션입니다. 지원자 수만큼의 문서만 포함하므로, 지원자 목록 조회 시 전체 본문 컬렉션을 스캔하지 않고 빠르게 조회할 수 있습니다.

**성능 vs 유연성 트레이드오프:**
- **성능**: 인덱스 컬렉션으로 지원자 목록 조회는 O(지원자 수), 본문 검색은 메타데이터 필터링으로 효율적
- **유연성**: 단일 본문 컬렉션 구조로 향후 복합 필터링(`candidate` + `project_type` 등) 확장 용이
- **ChromaDB 기본 기능 활용**: 메타데이터 필터링을 검색 단계에서 적극 활용하여 ChromaDB의 장점을 최대한 활용
